...
namespace DreamSoft
{
    class PLC_Tcp_CP
    {
        ...
        {
            if (IpIsOK(Config.Mac_C.IP_PLC, 1000))
            {
                try
                {
                    if (tct != null)
                    ...
                    tct = new TcpClient(Config.Mac_C.IP_PLC, 502);
                    skt = tct.Client;
                    skt.SendTimeout = skt.ReceiveTimeout = 1000;

                    //skt = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
                    //IPEndPoint iep = new IPEndPoint(IPAddress.Parse(Config.Soft.IPPLC), int.Parse(Config.Soft.PortPLC));
                    //skt.Connect(iep);
                    ...
                    if (skt.Connected)
                    {
                        ...
                        if (Config.Soft.Function == "O" && Config.Mac_C.PLC_Com == "Y")
                            PLC_Com_CP.SetSpeeds();
        ...
        static bool SendPLC(string send, string mark, out string response)
        {
            ...
            if (skt != null)
            {
                try
                {
                    ...
                    Thread.Sleep(50);
                    byte[] buffer_response = new byte[100];
                    skt.Receive(buffer_response, SocketFlags.None);
                    response = GetString(buffer_response);
                    if (int.Parse(response.Substring(8, 4)) > 0)
        ...
        static void SetSpeeds()
        {
            ...
            {
                //速度
                speed = GetReal(int.Parse(Config.Mac_C.Speed_Auto_X));
                ...
                speed = GetReal(int.Parse(Config.Mac_C.Speed_Auto_Z));
                ...
                speed = GetReal(int.Parse(Config.Mac_C.Speed_Auto_Plate));
                ...
                //加减速
                acc = GetReal(int.Parse(Config.Mac_C.Acc_Manual_X));
                ...
                acc = GetReal(int.Parse(Config.Mac_C.Acc_Manual_Z));
                ...
                acc = GetReal(int.Parse(Config.Mac_C.Acc_Manual_Plate));
                ...
                acc = GetReal(int.Parse(Config.Mac_C.Acc_Auto_X));
                ...
                acc = GetReal(int.Parse(Config.Mac_C.Acc_Auto_Z));
                ...
                acc = GetReal(int.Parse(Config.Mac_C.Acc_Auto_Plate));
                ...
                acc = GetReal(int.Parse(Config.Mac_C.Acc_Up_Plate));
            ...
            {
                //速度
                speed = GetReal(int.Parse(Config.Mac_C.Speed_Auto_Lift));
                ...
                //加速度
                acc = GetReal(int.Parse(Config.Mac_C.Acc_Manual_Lift));
                ...
                acc = GetReal(int.Parse(Config.Mac_C.Acc_Auto_Lift));
        ...
        public static void ExtramanManualMove(PLC_Tcp_AP.ExtramanMoveDir dir)
        {
            int speed = 0;
            int adr_Speed = 0;
            int adr_FX = 0;
            ...
            switch (dir)
            {
                case PLC_Tcp_AP.ExtramanMoveDir.Up:
                    adr_Speed = Speed_Z_Manual;
                    speed = int.Parse(Config.Mac_C.Speed_Manual_Z);
                    adr_FX = ExtramanManualMove_Z_FX;
                    fx = 1;
                    adr_On = ExtramanManualMove_Z_On;
                    break;
                case PLC_Tcp_AP.ExtramanMoveDir.Down:
                    adr_Speed = Speed_Z_Manual;
                    speed = int.Parse(Config.Mac_C.Speed_Manual_Z);
                    adr_FX = ExtramanManualMove_Z_FX;
                    fx = -1;
                    adr_On = ExtramanManualMove_Z_On;
                    break;
                case PLC_Tcp_AP.ExtramanMoveDir.Left:
                    adr_Speed = Speed_X_Manual;
                    speed = int.Parse(Config.Mac_C.Speed_Manual_X);
                    adr_FX = ExtramanManualMove_X_FX;
                    fx = 1;
                    adr_On = ExtramanManualMove_X_On;
                    break;
                case PLC_Tcp_AP.ExtramanMoveDir.Right:
                    adr_Speed = Speed_X_Manual;
                    speed = int.Parse(Config.Mac_C.Speed_Manual_X);
                    adr_FX = ExtramanManualMove_X_FX;
                    fx = -1;
                    adr_On = ExtramanManualMove_X_On;
                    break;
                case PLC_Tcp_AP.ExtramanMoveDir.XStop:
                    adr_Speed = Speed_X_Manual;
                    adr_On = ExtramanManualMove_X_On;
                    break;
                case PLC_Tcp_AP.ExtramanMoveDir.ZStop:
                    adr_Speed = Speed_Z_Manual;
                    adr_On = ExtramanManualMove_Z_On;
                    break;
            }
            ...
            string s = "";
            //方向
            if (dir != PLC_Tcp_AP.ExtramanMoveDir.XStop && dir != PLC_Tcp_AP.ExtramanMoveDir.ZStop)
            {
                s = Get16String(fx, 4);
        ...
        public static bool TrackErrorResetIsOK(PLC_Tcp_AP.TrackType type)
        {
            bool result = false;
            string send;
            string response;
            if (type == PLC_Tcp_AP.TrackType.X)
            {
        ...
        public static void TrackErrorReset(PLC_Tcp_AP.TrackType type)
        {
            int adr = 0;
            switch (type)
            {
                case PLC_Tcp_AP.TrackType.X:
                    adr = TrackReset_X;
                    break;
                case PLC_Tcp_AP.TrackType.Z:
                    adr = TrackReset_Z;
                    break;
            }
        ...
        public static void PlateManualMove(PLC_Tcp_AP.PlateType type, PLC_Tcp_AP.PlateMoveDir dir)
        {
            int adr_FX = 0;
            int fx = 0;
            int adr_speed = 0;
            ...
            switch (dir)
            {
                case PLC_Tcp_AP.PlateMoveDir.Up:
                    fx = 1;
                    speed = int.Parse(Config.Mac_C.Speed_Manual_Plate);
                    break;
                case PLC_Tcp_AP.PlateMoveDir.Down:
                    fx = -1;
                    speed = int.Parse(Config.Mac_C.Speed_Manual_Plate);
                    break;
                case PLC_Tcp_AP.PlateMoveDir.Stop:
                    fx = 0;
                    break;
            }
            string send = "";
            string response;
            //方向
            if (dir != PLC_Tcp_AP.PlateMoveDir.Stop)
            {
        ...
        public static bool PlateOriginResetState(PLC_Tcp_AP.PlateType type)
        {
            bool result = true;
            int adr = 0;
            //推药板原点复位状态
        ...
        //读取推药板当前脉冲
        public static float ReadPlatePulse(PLC_Tcp_AP.PlateType type)
        {
            float result = 0;
            int adr = 0;
            switch (type)
        ...
        //加药仓算数量上推
        public static void PlateMoveUp(PLC_Tcp_AP.PlateType type)
        {
            int adr_Speed = 0;
            int adr_On = 0;
            switch (type)
            ...
            string response;
            string speed = GetReal(int.Parse(Config.Mac_C.Speed_Up_Plate));
        ...
        //加药仓算数量上推是否完成
        public static bool PlateMoveUpIsOK(PLC_Tcp_AP.PlateType type)
        {
            bool result = false;
            int adr = 0;
            switch (type)
        ...
        public static void PlateAutoMoveToPulse(PLC_Tcp_AP.PlateType type, float pulse)
        {
        ...
        static void PlateAutoMoveToPulseOn(PLC_Tcp_AP.PlateType type)
        {
            int adr = 0;
            switch (type)
            {
        ...
        public static void PlateAutoMoveToPulseOFF(PLC_Tcp_AP.PlateType type)
        {
            int adr = 0;
            switch (type)
            {
        ...
        public static bool PlateAutoMoveToPulseIsOK(PLC_Tcp_AP.PlateType type)
        {
            bool result = false;
            int adr = 0;
            switch (type)
        ...
        public static bool PlateErrorResetIsOK(PLC_Tcp_AP.PlateType type)
        {
            bool result = false;
            int adr = 0;
            switch (type)
        ...
        public static void PlateErrorReset(PLC_Tcp_AP.PlateType type)
        {
            int adr = 0;
            switch (type)
            {
        ...
        //仓位报警代码
        public static string ReadPlateErrorCode(PLC_Tcp_AP.PlateType type)
        {
            string result = "";
            //报警代码
            int adr = 0;
        ...
        public static int ReadPlateRecord(PLC_Tcp_AP.PlateType type)
        {
            int result = 0;
            int adr = 0;
            switch (type)
        ...
        //加药计数清零
        public static void ResetPlateRecord(PLC_Tcp_AP.PlateType type)
        {
            int adr = 0;
            switch (type)
            {
        ...
        //    return result;
        //}
        public static bool PlateStateIsOK(PLC_Tcp_AP.PlateType type)
        {
            bool result = true;
            int adr = 0;
            switch (type)
        ...
        public static void LiftManualMove(PLC_Tcp_AP.LiftMoveDir dir)
        {
            int fx = 0;
            int speed = 0;
            switch (dir)
            {
                case PLC_Tcp_AP.LiftMoveDir.Up:
                    fx = 1;
                    speed = int.Parse(Config.Mac_C.Speed_Manual_Lift);
                    break;
                case PLC_Tcp_AP.LiftMoveDir.Down:
                    fx = -1;
                    speed = int.Parse(Config.Mac_C.Speed_Manual_Lift);
                    break;
                case PLC_Tcp_AP.LiftMoveDir.Stop:
                    fx = 0;
                    break;
            }
            string send = "";
            string response;
            if (dir != PLC_Tcp_AP.LiftMoveDir.Stop)//方向
            {
        ...
        //提升机运行到指定位置
        public static void LiftAutoMoveToPos(PLC_Tcp_AP.MovePos pos)
        {
            float value = 0;
            switch (pos)
            {
                case PLC_Tcp_AP.MovePos.Top:
                    value = float.Parse(Config.Mac_C.Pulse_Lift_Top);
                    break;
                case PLC_Tcp_AP.MovePos.Up:
                    value = float.Parse(Config.Mac_C.Pulse_Lift_Up);
                    break;
                case PLC_Tcp_AP.MovePos.Down:
                    value = float.Parse(Config.Mac_C.Pulse_Lift_Down);
                    break;
                case PLC_Tcp_AP.MovePos.Meet:
                    value = float.Parse(Config.Mac_C.Pulse_Lift_Meet);
                    break;
            }
        ...
        public static bool LiftOnMeet()
        {
            ...
            float meetPulse = float.Parse(Config.Mac_C.Pulse_Lift_Meet);
        ...
        public static void TransferBeltMove(PLC_Tcp_AP.TransferBeltMoveType type)
        {
            string value = "";
            switch (type)
            {
                case PLC_Tcp_AP.TransferBeltMoveType.Left:
                    value = "FF00";
                    break;
                case PLC_Tcp_AP.TransferBeltMoveType.Right:
                    value = "FF00";
                    break;
                case PLC_Tcp_AP.TransferBeltMoveType.Stop:
                    value = "0000";
                    break;
            }
        ...
        public static void TopBeltMove(PLC_Tcp_AP.TopBeltMoveType type)
        {
            string value = "";
            switch (type)
            {
                case PLC_Tcp_AP.TopBeltMoveType.Turn:
                    value = "FF00";
                    break;
                case PLC_Tcp_AP.TopBeltMoveType.Stop:
                    value = "0000";
                    break;
            }
...
where di.drugonlycode=dp.drugonlycode and dp.maccode='{0}' and dp.poscode='{1}'";
            ...
            if (!DPJ_CP.DCTMoveDownSingle(master, dct, t))
            {
                //弹跳失败
            }
        }
        ...
        public static int ReadRecordSingle(string poscode)
        {
            ...
            int dct = int.Parse(poscode.Substring(3, 2));
            int num;
            if (!DPJ_CP.ReadRecordSingle(master, dct, out num))
            {
            }
            return num;
        }
        ...
        public static void ClearRecordSingle(string poscode)
        {
            int master = int.Parse(poscode.Substring(1, 2));
            int dct = int.Parse(poscode.Substring(3, 2));
            if (!DPJ_CP.ClearRecordSingle(master, dct))
            {
            }
        }
        //多个计数器清零
        ...
        public static int PDNum(string posCode, bool isNull)
        {
            ...
            //右臂盘点
            if (int.Parse(posCode.Substring(0, 1)) == Config.Mac_C.Count_Unit && int.Parse(posCode.Substring(3, 2)) > Config.Mac_C.MaxCol)
            {
            ...
            //左臂盘点
            else if (posCode.Substring(0, 1) == "1" && int.Parse(posCode.Substring(3, 2)) < Config.Mac_C.MinCol)
            {
                sql = "select pulsex,pulsez from pos_pulse where maccode='{0}' and poscode='{1}' and pulsetype='{2}' and pulselr='{3}'";
            ...
            else
            {
                ...
                {
                    float nowx = ReadExtramanPulseX();
                    float nowz = ReadExtramanPulseZ();
            ...
            if (x != 0f && z != 0f)
            {
                //运行到盘点位置
                ChangeAdd(1);
                if (ExtramanAutoMoveToPulse(x, z))
                {
                    DateTime timeBegin = DateTime.Now;
                    Thread.Sleep(200);
                    while (!ExtramanAutoMoveIsOK())
                    {
                    ...
                    if (DateTime.Now <= timeBegin.AddSeconds(Config.Mac_C.WaitTime_Auto_Extraman))
                    {
                        ...
                        int laserData = 0;
                        if (dir == "L")
                            laserData = Laser.ReadLaserData(PLC_Tcp_AP.LaserType.Left);
                        else laserData = Laser.ReadLaserData(PLC_Tcp_AP.LaserType.Right);
                        if (laserData >= 0)
                        {
                            if (!isNull)
                            {
                                ...
                                if (!string.IsNullOrEmpty(s))
                                {
                                    ...
                                    if (float.TryParse(s, out length))
                                    {
                                        if (length > 0)
                                        {
                                            int n = (int)Math.Round((Config.Mac_C.Length_Laser - laserData) / length);
                                            if (n >= 0)
                                                result = n;
                                        }
                                    }
                            ...
                            {
                                if (Math.Abs(Config.Mac_C.Length_Laser - laserData) > 50)
                                    result = 1;
                                else result = 0;
                            }
                        }
        ...
        static void GetPDPulseFromAdd(string posCode, string dir, ref float x, ref float z)
        {
            ...
            if (dtAdd != null && dtAdd.Rows.Count > 0)
            {
                ...
                {
                    if (dir == "L")
                        x = float.Parse(dtAdd.Rows[0][0].ToString()) - (float.Parse(v) / 2 + Config.Mac_C.Laser_Offset_Left_X) * Config.Mac_C.OneMMPulse_Extraman_X;
                    else
                        x = float.Parse(dtAdd.Rows[0][0].ToString()) + (float.Parse(v) / 2 + Config.Mac_C.Laser_Offset_Right_X) * Config.Mac_C.OneMMPulse_Extraman_X;

                    z = float.Parse(dtAdd.Rows[0][1].ToString()) - (Config.Mac_C.Laser_Offset_Right_Z - 8) * Config.Mac_C.OneMMPulse_Extraman_Z;
                }
            }
        }
