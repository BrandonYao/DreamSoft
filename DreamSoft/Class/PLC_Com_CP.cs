...
namespace DreamSoft
{
    class PLC_Com_CP
    {
        ...
        public static void Initial()
        {
            ...
            spPLC = new SerialPort(Config.Mac_C.Port_PLC, 9600, Parity.Even, 7, StopBits.One);
        ...
        {
            BaffleAutoMoveByPulse_Lift(int.Parse(Config.Mac_C.Pulse_Baffle_Open));
        }
        public static void BaffleClose()
        {
            BaffleAutoMoveByPulse_Lift(int.Parse(Config.Mac_C.Pulse_Baffle_Close));
        }
        ...
        public static void BaffleAutoMoveByPulse_Up_Lift(int pulse)
        {
            ...
            string send = RegDMW + PLC_Tcp_CP.GetAdrD_1000(BaffleAutoMovePulse_Up_Lift) + "0002" + "04" + s.Substring(4, 4) + s.Substring(0, 4);
            string response;
            if (SendPLC(send, "挡板自动运行脉冲", out response))
            {
                send = RegMSW + PLC_Tcp_CP.GetAdrM(BaffleAutoMovePulse_Up_Lift_On) + "FF00";
        ...
        {
            bool result = false;
            string send = RegDR + PLC_Tcp_CP.GetAdrD_1000(BaffleAutoMoveStatus_Down) + "0001";
            string response;
            if (SendPLC(send, "挡板自动运行状态", out response))
            {
                int i = response.IndexOf(":");
        ...
        public static bool BaffleOnOpen()
        {
            ...
            int openPulse = int.Parse(Config.Mac_C.Pulse_Baffle_Open);
        ...
        public static bool BaffleOnClose()
        {
            bool result = false;
            int nowPulse = ReadBafflePulse_Lift();
            int closePulse = int.Parse(Config.Mac_C.Pulse_Baffle_Close);
        ...
        {
            bool result = false;
            string send = RegDR + PLC_Tcp_CP.GetAdrD_1000(BaffleManualMoveStatus_Up) + "0001";
            string response;
            ...
            if (!result)
            {
                send = RegDR + PLC_Tcp_CP.GetAdrD_1000(BaffleAutoMoveStatus_Up) + "0001";
                if (SendPLC(send, "出药挡板状态", out response))
                {
                    int i = response.IndexOf(":");
                    if (i >= 0)
