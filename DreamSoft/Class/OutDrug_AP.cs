...
namespace DreamSoft.Class
{
    class OutDrug_AP
    {
        ...
        //构造函数
        public OutDrug_AP(PLC_Tcp_AP.MovePos type, DataTable dt, bool bc, string presc, int pc, DataTable dtD)
        {
            OutPosition = type;
            dtToOut = dt;
            BC = bc;
        ...
        public void Out()
        {
            ...
            if (NeedOut(dtToOut, out poss))
            {
                ...
                if (!PLC_Tcp_AP.LiftOnMeet())
                {
                    PLC_Tcp_AP.LiftAutoMoveToPos(PLC_Tcp_AP.MovePos.Meet);
                }
                if (!PLC_Com_AP.BaffleOnClose())
                {
                    PLC_Com_AP.BaffleClose();
                }

                //ThrowMsg("出药准备");
                PLC_Tcp_AP.TransferBeltMove(PLC_Tcp_AP.TransferBeltMoveType.Right);
                Thread.Sleep(Config.Mac_A.WaitTime_Start);

                //ThrowMsg("等待提升机到位");
                DateTime timeBegin = DateTime.Now;
                Thread.Sleep(200);
                while (!PLC_Tcp_AP.LiftOnMeet())
                {
                    if (DateTime.Now > timeBegin.AddSeconds(Config.Mac_A.WaitTime_Auto_Lift))
                    {
                        ThrowMsg("提升机未到位");
                        IsOut = false;
                        return;
                ...
                Thread.Sleep(200);
                while (!PLC_Com_AP.BaffleOnClose())
                {
        ...
        outStart:
            ...
            //计数清零
            PLC_Tcp_AP.ClearRecordMutil(poss);
            Dictionary<string, int> dicsOldRecord = PLC_Tcp_AP.ReadRecordMutil(poss);
            Dictionary<string,int> dicsNewRecord;
            //ThrowMsg("开始出药");
            while (NeedOut(dtToOut, out poss))
            {
                //ThrowMsg("出药动作");
                PLC_Tcp_AP.DCTMoveDownMutil(poss);
                //ThrowMsg("等待计数");
                Thread.Sleep(1000);
                dicsNewRecord = PLC_Tcp_AP.ReadRecordMutil(poss);
                //ThrowMsg("更新出药数据表");
            ...
            if (haveOut)
                Thread.Sleep(Config.Mac_A.WaitTime_Stop);

            PLC_Tcp_AP.TransferBeltMove(PLC_Tcp_AP.TransferBeltMoveType.Stop);
            if (haveOut)
            {
                if (OutPosition == PLC_Tcp_AP.MovePos.Top)
                    PLC_Tcp_AP.TopBeltMove(PLC_Tcp_AP.TopBeltMoveType.Turn);

                //ThrowMsg("送至出药口");
                PLC_Tcp_AP.LiftAutoMoveToPos(OutPosition);
                DateTime timeBegin = DateTime.Now;
                Thread.Sleep(500);
                while (!PLC_Tcp_AP.LiftAutoMoveIsOK())
                {
                    if (DateTime.Now > timeBegin.AddSeconds(Config.Mac_A.WaitTime_Auto_Lift))
                    {
                        ThrowMsg("提升机未到位");
                ...
                //ThrowMsg("出药");
                PLC_Com_AP.BaffleOpen();
                while (!PLC_Com_AP.BaffleOnOpen())
                    Thread.Sleep(200);
                Thread.Sleep(Config.Mac_A.OpenTime_Baffle);
                PLC_Com_AP.BaffleClose();
