...
namespace DreamSoft
{
    ...
    /// </summary>
    public partial class UCDebug_Add_CP : UserControl
    {
        CSHelper.Msg csMsg = new CSHelper.Msg();
        CSHelper.COMM csCOMM = new CSHelper.COMM();
        ...
        public UCDebug_Add_CP()
        {
            InitializeComponent();

            cbUnitCode.Items.Clear();
            for (int i = 1; i <= Config.Mac_C.Count_Unit; i++)
            {
                cbUnitCode.Items.Add(i.ToString());
            } 
            cbLayerCode.Items.Clear();
            for (int i = 1; i <= Config.Mac_C.Count_Lay; i++)
            {
        ...
        private void SavedPulse(int x, int z)
        {
            ...
            if (!string.IsNullOrEmpty(u) && !string.IsNullOrEmpty(l) && !string.IsNullOrEmpty(c))
            {
                ...
                csSql.ExecuteSelect(sql, Config.Soft.ConnString, out dt);
                if (dt.Rows.Count > 0)
                    sql = "update pos_pulse set pulsex=" + x + ",pulse=" + z + " where maccode='" + Config.Soft.MacCode + "' and poscode='" + poscode + "' and pulsetype='" + type + "' and pulselr='" + dir + "'";
                else sql = "insert into pos_pulse values ('" + Config.Soft.MacCode + "','" + poscode + "','" + type + "','" + dir + "'," + x + "," + z + ")";
        ...
        private void btExtraman_Manual_Up_PreviewMouseDown(object sender, MouseButtonEventArgs e)
        {
            ...
            PLC_Tcp_CP.ExtramanManualMove(PLC_Tcp_AP.ExtramanMoveDir.Up);
        }
        //下降
        private void btExtraman_Manual_Down_PreviewMouseDown(object sender, MouseButtonEventArgs e)
        {
            PLC_Tcp_CP.ChangeAdd(0);
            PLC_Tcp_CP.ExtramanManualMove(PLC_Tcp_AP.ExtramanMoveDir.Down);
        }
        //停止
        private void btExtraman_Manual_Z_PreviewMouseUp(object sender, MouseButtonEventArgs e)
        {
            PLC_Tcp_CP.ExtramanManualMove(PLC_Tcp_AP.ExtramanMoveDir.ZStop);
            Thread.Sleep(500);
            tbNowZ.Text = PLC_Tcp_CP.ReadExtramanPulseZ().ToString();
        }
        #endregion
        ...
        {
            PLC_Tcp_CP.ChangeAdd(0);
            PLC_Tcp_CP.ExtramanManualMove(PLC_Tcp_AP.ExtramanMoveDir.Left);
        }
        //向右
        ...
        {
            PLC_Tcp_CP.ExtramanManualMove(PLC_Tcp_AP.ExtramanMoveDir.XStop);
            Thread.Sleep(500);
            tbNowX.Text = PLC_Tcp_CP.ReadExtramanPulseX().ToString();
        }
        #endregion
        ...
        private void btSavePulse_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(u) && !string.IsNullOrEmpty(l) && !string.IsNullOrEmpty(c))
            {
                ...
                if ((bool)chkRow.IsChecked)
                {
                    //同层一起设定Z轴脉冲
                    //所有单元
                    for (int unit = 1; unit <= Config.Mac_C.Count_Unit; unit++)
                    {
                        for (int col = 1; col <= Config.Mac_C.Count_Col; col++)
                        {
                ...
                {
                    //同列一起设定X轴脉冲 
                    for (int row = 1; row <= Config.Mac_C.Count_Lay; row++)
                    {
        ...
        {
            PLC_Tcp_CP.ChangeAdd(0);
            PLC_Tcp_CP.PlateManualMove(PLC_Tcp_AP.PlateType.Left, PLC_Tcp_AP.PlateMoveDir.Up);
        }
        //推药板向下
        private void btPlateDownL_MouseDown(object sender, MouseButtonEventArgs e)
        {
            PLC_Tcp_CP.ChangeAdd(0);
            PLC_Tcp_CP.PlateManualMove(PLC_Tcp_AP.PlateType.Left, PLC_Tcp_AP.PlateMoveDir.Down);
        }
        //推药板停止
        private void btPlateL_MouseUp(object sender, MouseButtonEventArgs e)
        {
            PLC_Tcp_CP.PlateManualMove(PLC_Tcp_AP.PlateType.Left, PLC_Tcp_AP.PlateMoveDir.Stop);
            Thread.Sleep(500);
            tbNowL.Text = PLC_Tcp_CP.ReadPlatePulse(PLC_Tcp_AP.PlateType.Left).ToString();
        }
        //推药板自动运行
        private void btRunL_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                PLC_Tcp_CP.PlateAutoMoveToPulse(PLC_Tcp_AP.PlateType.Left, pp);
                DateTime timeBegin = DateTime.Now;
                Thread.Sleep(200);
                while (!PLC_Tcp_CP.PlateAutoMoveToPulseIsOK(PLC_Tcp_AP.PlateType.Left))
                {
        ...
        {
            PLC_Tcp_CP.ChangeAdd(0);
            PLC_Tcp_CP.PlateManualMove(PLC_Tcp_AP.PlateType.Right, PLC_Tcp_AP.PlateMoveDir.Up);
        }
        //推药板向下
        ...
        {
            PLC_Tcp_CP.PlateManualMove(PLC_Tcp_AP.PlateType.Right, PLC_Tcp_AP.PlateMoveDir.Stop);
            Thread.Sleep(500);
            tbNowR.Text = PLC_Tcp_CP.ReadPlatePulse(PLC_Tcp_AP.PlateType.Right).ToString();
        }
        //推药板自动运行
        ...
        {
            float x = PLC_Tcp_CP.ReadPlatePulse(PLC_Tcp_AP.PlateType.Left);
        ...
        private void RB_Checked(object sender, RoutedEventArgs e)
        {
            if ((bool)rbHandL.IsChecked)
            {
                ...
                for (int i = 1; i <= Config.Mac_C.MaxCol; i++)
                {
            ...
            {
                cbColumnCode.Items.Clear();
                for (int i = Config.Mac_C.MinCol; i <= Config.Mac_C.Count_Col; i++)
                {
                    cbColumnCode.Items.Add(i.ToString().PadLeft(2, '0'));
                }
            }
